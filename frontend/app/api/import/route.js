...
export async function POST(request) {
  try {
    ...
    // --- Real Gemini AI Mapping (with graceful fallback on quota/model errors) ---
    try {
      const genAI = new GoogleGenerativeAI(apiKey);

      // Try lighter models first — they have higher free-tier quotas
      const modelsToTry = ["gemini-1.5-flash-8b", "gemini-1.5-flash", "gemini-2.0-flash"];
      let mappedLeads = null;
      let lastError = null;
      ...
      for (const modelName of modelsToTry) {
        try {
          ...
          mappedLeads = JSON.parse(response.response.text());
          console.log(`Success with model ${modelName}: ${mappedLeads.length} leads mapped.`);
          break; // success — stop trying other models
        } catch (modelErr) {
          lastError = modelErr;
          const msg = modelErr.message || "";
          // Only try next model on quota or not-found errors
          ...
          throw modelErr; // other errors — don't retry
        }
      }

      // If all models failed, fall back to mock mapper silently
      if (!mappedLeads) {
        console.warn("All Gemini models failed. Falling back to Mock AI mapper. Last error:", lastError?.message);
...
// Extracted mock mapper as a reusable function
function runMockMapper(fileData) {
  const mappedLeads = fileData.map((row) => {
    ...
    return {
      ...
      lead_owner: rawLeadOwner || "Arul Nandhi",
      crm_status,
      crm_note: isSkipped ? "Skipped: both email and phone number are missing." : "Mapped via AI fallback.",
      data_source,
      possession_time: "",
      description: rawDescription || "CSV upload",
    };
